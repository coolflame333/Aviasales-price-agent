...
В `live_searches.json` лучше вставить URL, который вы получили после ручной настройки фильтров на Aviasales: даты, прямые рейсы, нужные времена вылета и возврата. Поле `max_price` задает цену покупки/срочного уведомления. Поле `min_price` отсекает маленькие суммы из интерфейса страницы, которые не являются ценой билета; для Москва-Стамбул туда-обратно разумно оставить `10000`. Поле `outbound_departure_latest` дополнительно отбрасывает видимые рейсы с вылетом позже заданного времени, например `"12:00"`. Поле `excluded_origin_airports` отбрасывает вылеты из нежелательных аэропортов, например `["ZIA"]` для Жуковского. Поле `excluded_carriers` отбрасывает нежелательных перевозчиков, например `["Победа"]`. Если точный фильтр не сохраняется в URL, live-режим все равно откроет страницу и даст кнопку для быстрой ручной проверки.
